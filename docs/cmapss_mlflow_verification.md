...
Parameters `skip_lstm`, `gbm_max_train_rows`, and `anomaly_max_train_rows` appear on each `FD00X_phase3_summary` run.
...
3. Open each **`FD00X_phase3_summary`** run:
   ...
   - **Metrics:** `test_rmse`, `test_rul_score`, `winner_val_*`
   - **Artifacts:** `phase3_summary.json` (full JSON)

Nested runs under each summary: `FD00X_rul_rf`, `FD00X_rul_gbm`, `FD00X_rul_lstm`, `FD00X_failure_30_gbm`, `FD00X_failure_72_gbm`, `FD00X_anomaly_iforest`.
